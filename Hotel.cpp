...
<< "Wybor: ";
    ...
    void ListRoomsFlow(const Hotel& hotel)
    {
        ...
        else
        {
            ...
            for (const auto& r : hotel.GetRooms())
            {
                ...
                std::cout << "  " << *r << " | Status: " << (avail ? "WOLNY" : "ZAJETY") << "\n";
            }
        }
    }
    ...
    void ApplyPromoCodeFlow(Hotel& hotel)
    {
        ...
        // Ustawiamy flage przed zapisem uslugi, aby baza danych zapisala promo_applied = 1.
    ...
    void RemovePromoCodeFlow(Hotel& hotel)
    {
        ...
        hotel.RemovePromoFromReservation(resId);
...
int main()
{
    ...
    Database db;
    if (db.Connect(/*host*/"tcp://127.0.0.1:3306", /*user*/"root", /*password*/"", /*schema*/"hotelBaza"))
    {
        hotel.SetDatabase(&db);
        db.LoadInto(hotel);
    }

    // Jesli baza jest pusta lub niedostepna, ladujemy domyslny katalog pokoi.
    if (hotel.GetRooms().empty())
    {
        if (db.IsConnected())
            std::cout << "[DB] Brak pokoi w bazie - laduje katalog domyslny.\n";
        else
            std::cout << "[DB] Brak polaczenia z baza - tryb pamieciowy (dane nie beda zapisywane).\n";
        SeedRooms(hotel);
    }
